...
def health_check_offerer_status():
    does_enriched_offerer_data_exist = partial(is_enriched_materialized_view_queryable,
...
def health_check_user_status():
    does_enriched_user_data_exist = partial(is_enriched_materialized_view_queryable, materialized_view_name='enriched_user_data')
...
def health_check_stock_status():
    does_enriched_stock_data_exist = partial(is_enriched_view_queryable, view_name='enriched_stock_data')
...
def health_check_offer_status():
    does_enriched_offer_data_exist = partial(is_enriched_view_queryable, view_name='enriched_offer_data')
...
def write_enriched_data():
    ...
    if token == bastion_token:
        create_enriched_data_views(app_name)
        return '', 200
    return '', 401
