...
def is_enriched_materialized_view_queryable(healthcheck_session_maker: sessionmaker, materialized_view_name: str):
    is_enriched_data_present = False
    health_check_session = healthcheck_session_maker()
    try:
        is_enriched_data_present = does_materialize_view_exist(health_check_session, materialized_view_name)
...
def does_enriched_offerer_contain_data(healthcheck_session_maker: sessionmaker) -> bool:
    ...
    health_check_session = healthcheck_session_maker()
    if is_enriched_materialized_view_queryable(healthcheck_session_maker, 'enriched_offerer_data'):
        try:
...
def does_enriched_users_contains_data(healthcheck_session_maker: sessionmaker) -> bool:
    ...
    health_check_session = healthcheck_session_maker()
    if is_enriched_materialized_view_queryable(healthcheck_session_maker, 'enriched_user_data'):
        try:
