...
def _get_number_of_bookings_per_venue(venue_id: str) -> str:
    return f'''
    SELECT
        count(booking.id)
    FROM offer
...
ENRICHED_VENUE_QUERY = query = f'''
    SELECT
        ...
        ,({_get_number_of_bookings_per_venue("venue.id")}) AS "Nombre total de réservations"
        ,0 AS "Nombre de réservations non annulées"
        ,0 AS "Nombre de réservations validées"
        ,0 AS "Date de création de la première offre"
        ,0 AS "Date de création de la dernière offre"
        ,0 AS "Nombre d'offres créées"
        ,0 AS "Chiffre d'affaires théorique réalisé"
        ,0 AS "Chiffre d'affaires réel réalisé"
    FROM venue
...
def create_enriched_venue_view() -> None:
    query = f'CREATE OR REPLACE VIEW enriched_venue_data AS ({ENRICHED_VENUE_QUERY})'
