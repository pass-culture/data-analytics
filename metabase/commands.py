...
def switch_metabase_database_connection(
    database_name: str, user_name: str, password: str, local: bool
):
    ...
    db_details["ssl"] = False if local else True
    db_settings = {"details": db_details, "name": database_name, "engine": "postgres"}
    requests.put(
...
def get_app_name_for_restore():
    session_id = configure_new_metabase_session(
        os.environ.get("METABASE_USER_NAME"), os.environ.get("METABASE_PASSWORD")
    )
    host = get_connected_database_host_name(
        os.environ.get("METABASE_DBNAME"), session_id
    )
