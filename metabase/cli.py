...
@click.command(
    "show",
    help="Show app name that is not linked to Metabase. The restore has to be done on this app",
)
...
def switch_host_for_restore(local: bool):
    switch_metabase_database_connection(
        os.environ.get("METABASE_DBNAME"),
        os.environ.get("METABASE_USER_NAME"),
