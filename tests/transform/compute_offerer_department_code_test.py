...
from transform.compute_offerer_department_code import (
    get_offerer_with_departement_code_dataframe,
)
...
class GetOffererWithDepartementCodeDataframe:
    def test_should_return_empty_dataframe_when_given_dataframe_is_empty(self):
        ...
        # When
        result = get_offerer_with_departement_code_dataframe(
            empty_postal_code_dataframe
        )
    ...
    def test_should_return_dataframe_with_department_code_column(self):
        # Given
        postal_code_dataframe = pandas.DataFrame(
            data={"id": [1, 2], "postalCode": ["75003", "97459"]}
        )
        expected_dataframe = pandas.DataFrame(
            data={"id": [1, 2], "department_code": ["75", "974"]}
        )
