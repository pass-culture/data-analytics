...
from metabase.commands import (
    configure_new_metabase_session,
    get_connected_database_id,
    get_connected_database_host_name,
    get_db_details_by_app_name,
    get_app_name_for_restore,
    switch_metabase_database_connection,
)
...
class ConfigureNewMetabaseSessionTest:
    ...
    def test_should_return_session_id_from_api_session(self, mock_request):
        ...
        mock_request.assert_called_once_with(
            "metabase.example.com/api/session/",
            json={"username": "admin.metabase@example.com", "password": "password"},
        )
...
class GetDumpTableIdTest:
    ...
    def test_should_return_table_id_in_metabase_from_api_database(self, mock_request):
        ...
        mock_request.assert_called_once_with(
            "metabase.example.com/api/database/",
...
class GetDumpTableInformationTest:
    ...
    @patch("metabase.commands.requests.get")
    def test_should_return_host_from_api_database_table_id_in_metabase(
        self, mock_request, mock_get_connected_database_id
    ):
        # Given
        session_id = 42
        request_json = {
        ...
        mock_request.assert_called_once_with(
            "metabase.example.com/api/database/table_id_in_metabase",
            headers={"cookie": "metabase.SESSION=42"},
        )
...
class GetAppNameForRestoreTest:
    ...
    def test_should_return_green_app_name_when_blue_db_is_on_metabase(
        self, mock_configure_new_metabase_session, mock_get_connected_database_host_name
    ):
        # Given
        mock_configure_new_metabase_session.return_value = "session_id"
        mock_get_connected_database_host_name.return_value = (
            "db_blue.postgresql.example.com"
        )
    ...
    @patch("metabase.commands.get_connected_database_host_name")
    @patch("metabase.commands.configure_new_metabase_session")
    def test_should_return_blue_app_name_when_green_db_is_on_metabase(
        self, mock_configure_new_metabase_session, mock_get_connected_database_host_name
    ):
        # Given
        mock_configure_new_metabase_session.return_value = "session_id"
        mock_get_connected_database_host_name.return_value = (
            "db_green.postgresql.example.com"
        )
...
class SwitchMetabaseDatabaseConnectionTest:
    ...
    @patch("metabase.commands.get_connected_database_id")
    @patch("metabase.commands.configure_new_metabase_session")
    def test_should_switch_connection_to_dump_table(
        self,
        mock_configure_new_metabase_session,
        mock_get_connected_database_id,
        mock_get_app_name_for_restore,
        mock_get_db_details_by_app_name,
        mock_request,
    ):
        # Given
        ...
        # When
        switch_metabase_database_connection(
            table_name, user_name, password, local=False
        )

        # Then
        mock_request.assert_called_once_with(
            "metabase.example.com/api/database/table_id",
    ...
    @patch("metabase.commands.get_connected_database_id")
    @patch("metabase.commands.configure_new_metabase_session")
    def test_should_not_use_ssl_connection_when_url_is_local(
        self,
        mock_configure_new_metabase_session,
        mock_get_connected_database_id,
        mock_get_app_name_for_restore,
