...
class OffererQueriesTest:
    ...
    class GetFirstStockCreationDatesQueryTest:
        def test_should_return_the_creation_date_of_the_offer_s_first_stock(self):
            ...
            with ENGINE.connect() as connection:
                first_stock_dates = pandas.read_sql(
                    query, connection, index_col="offerer_id"
                )
            assert first_stock_dates.loc[
                1, "Date de création du premier stock"
            ] == datetime(2019, 12, 1)
    ...
    class GetFirstBookingCreationDatesQueryTest:
        def test_should_return_the_creation_date_of_the_offer_s_first_booking(self):
            ...
            create_booking(
                ...
                stock_id=1,
                id=1,
                date_created=first_booking_date,
                token="123456",
            )
            create_booking(
                user_id=1,
                stock_id=1,
                id=2,
                date_created=second_booking_date,
                token="AZERTY",
            )
            ...
            # Then
            with ENGINE.connect() as connection:
                first_booking_dates = pandas.read_sql(
                    query, connection, index_col="offerer_id"
                )
            assert (
                first_booking_dates.loc[1, "Date de première réservation"]
                == first_booking_date
            )
    ...
    class GetNumberOfOffersQueryTest:
        def test_should_return_the_number_of_offers(self):
            ...
            # Then
            with ENGINE.connect() as connection:
                number_of_offers = pandas.read_sql(
                    query, connection, index_col="offerer_id"
                )
    ...
    class GetNumberOfBookingsNotCancelledQueryTest:
        def test_should_return_the_number_of_bookings_not_cancelled(self):
            ...
            create_booking(
                user_id=2, stock_id=1, id=3, token="6YHA08", is_cancelled=True
            )
            ...
            # Then
            with ENGINE.connect() as connection:
                number_of_bookings_not_cancelled = pandas.read_sql(
                    query, connection, index_col="offerer_id"
                )
            assert (
                number_of_bookings_not_cancelled.loc[
                    1, "Nombre de réservations non annulées"
                ]
                == 2
            )
    ...
    class GetNumberOfVenuesPerOffererQueryTest:
        def test_should_return_the_number_of_venue(self):
            ...
            # Then
            with ENGINE.connect() as connection:
                number_of_venue = pandas.read_sql(
                    query, connection, index_col="offerer_id"
                )
    ...
    class GetNumberOfVenueWithOfferPerOffererQueryTest:
        def test_should_return_the_number_of_venue_with_at_least_one_offer(self):
            ...
            # Then
            with ENGINE.connect() as connection:
                number_of_venue_with_offer = pandas.read_sql(
                    query, connection, index_col="offerer_id"
                )
