...
from read.postgresql_database.health_check_queries import does_enriched_offerer_data_exists, does_enriched_user_data_exists, does_enriched_stock_data_exists, \
    does_enriched_offer_data_exists, does_enriched_offerer_contains_data, does_enriched_stocks_contains_data, does_enriched_users_contains_data, \
    does_enriched_offer_contains_data
...
class DoesEnrichedUserDataExistsTest:
    @pytest.fixture(autouse=True)
...
class DoesEnrichedStockDataExistsTest:
    @pytest.fixture(autouse=True)
    def setup_class(self, app):
        clean_database(app)
        clean_views()
...
class DoesEnrichedOfferDataExistsTest:
    @pytest.fixture(autouse=True)
    def setup_method(self, app):
        yield
        clean_database(app)
    ...
    def test_should_return_false_when_no_table_exists(self, app):
        # When
        result = does_enriched_offer_data_exists()
...
class DoesEnrichedOffererSourceContainsDataTest:
    @pytest.fixture(autouse=True)
    def setup_class(self, app):
        clean_database(app)
        clean_views()
...
class DoesEnrichedOfferSourceContainsDataTest:
    ...
    def test_should_return_false_when_contains_no_data(self, app):
        # When
        result = does_enriched_offer_contains_data()
