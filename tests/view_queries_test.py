...
class ViewQueriesTest:
    ...
    class CreateEnrichedUserViewTest:
        def test_should_create_enriched_user_data_view_with_columns(self, app):
            ...
            # Then
            expected_columns = ['Département', 'Date de remplissage du typeform', "Vague d'expérimentation", \
             "Date d'activation", 'Date de première connexion', 'Date de première réservation', \
             'Date de deuxième réservation', 'Date de première réservation dans 3 catégories différentes', \
             'Date de dernière recommandation', 'Nombre de réservations totales', \
             'Nombre de réservations non annulées', 'Ancienneté en jours', 'Montant réél dépensé', \
             'Montant théorique dépensé', 'Dépenses numériques', 'Dépenses physiques']
