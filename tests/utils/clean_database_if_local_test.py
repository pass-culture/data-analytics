...
class CleanDatabaseIfLocalTest:
    ...
    @patch(
        "utils.clean_database_if_local.LOCAL_DATABASE_URL",
        "postgresql://pass_culture:passq@localhost:5435/pass_culture",
    )
    @patch(
        "utils.clean_database_if_local.DATABASE_URL",
        "postgresql://pass_culture:passq@remote.host:5435/pass_culture",
    )
    def test_should_not_clean_database_when_url_is_remote(
        self, mock_clean_views, mock_clean_database
    ):
        # Given / When
    ...
    def test_should_clean_database_when_url_is_local(
        self, mock_clean_views, mock_clean_database
    ):
        # Given / When
        clean_database_if_local()
